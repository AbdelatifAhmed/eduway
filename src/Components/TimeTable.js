...
const Timetable = () => {
  ...
  const generateTimetable = () => {
    const timetableMatrix = Array.from({ length: 13 }, () =>
      Array(7).fill(null)
    );
  ...
  return (
    <div className="pad">
      <h2>Timetable</h2>
      <header
        className="d-flex flex-column flex-md-row justify-content-between fw-bold"
        style={{
          background: "#121431",
          color: "white",
          padding: "20px",
      ...
      >
        <div className="item mb-2 mb-md-0">
          Name: <span>{timetable?.nameEnglish}</span>
        </div>
        <div className="item mb-2 mb-md-0">
          Faculty: <span>{timetable?.facultysName}</span>
        </div>
        <div className="item mb-2 mb-md-0">
          Academic Year: <span>{timetable?.academyYearName}</span>
        </div>
      </header>
                  ...
                  if (cellData) {
                    if (cellData.isMain) {
                      ...
                      return (
                        <td
                          key={day}
                          title={convertTo12HourFormat(slot?.timing)}
                          rowSpan={span}
                          style={{
                            background:
                              slot.scheduleType === 1 ? "#FFD700" : "#ADFF2F",
                            padding: "10px",
                            borderRadius: "5px",
                            marginBottom: "5px",
                          }}
                        >
                          ...
                          <p>
                            {slot.scheduleType === 1 ? "Lecture" : "Session"}
                          </p>
                        </td>
                      );
                    } else {
