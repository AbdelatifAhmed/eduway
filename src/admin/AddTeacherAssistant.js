...
export default function AddTeacherAssistant() {
  ...
  const handelAddTeacherAssistant = async (event) => {
      ...
      await axios
        ...
        .then((response) => {
          if (response.status === 201) {
            Toast.fire({
              icon: "success",
              title: "Teacher Assistant added successfully",
            });
          }
        });
    } catch (err) {
  ...
  return (
    <div>
      <form onSubmit={handelAddTeacherAssistant}>
        <div
          style={{ marginLeft: "20px" }}
          className="d-flex  justify-content-between"
        >
          <h1 className="d-inline">Add Teacher Assistant </h1>
